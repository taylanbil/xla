...
struct MeshService::Impl {
  Impl(grpc::Config config) : impl(std::move(config)) {}
...
MeshService::MeshService(const string& address, grpc::Config config)
  ...
  builder.RegisterService(&impl_->impl);
...
MeshClient::MeshClient(const string& address) : impl_(new Impl()) {
  std::shared_ptr<::grpc::Channel> channel =
      ::grpc::CreateChannel(address, ::grpc::InsecureChannelCredentials());
  impl_->stub = grpc::MeshService::NewStub(channel);
}
